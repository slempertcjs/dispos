...
coll_df <- dsam %>%
  ...
  #create a new variable to store the latest disposition date associated with all steps in a cycle. if there are no disposition dates, use the latest step date, if there are no step event dates use the cycle date. 
  group_by(subject_id, cyc_date) %>%
  
  mutate(dispo_date = max(dispo_date, na.rm = T),
         dispo_date = if_else(dispo_date == -Inf, as_date(NA), dispo_date),
         stp_event_date = max(stp_event_date, na.rm = T),
         stp_event_date = if_else(stp_event_date == -Inf, as_date(NA), stp_event_date),
         cycle_dispo_date = case_when( !is.na(dispo_date) ~ dispo_date,
                                       !is.na(stp_event_date) ~ stp_event_date,
                                       T ~ cyc_date)) %>%
  ...
  mutate(max_arrest_hier = min(max_arrest_hier),
         max_court_hier = min(max_court_hier),
         max_conv_hier = min(max_conv_hier)) %>%
...
#filter out the final dispositions in 2019 (use dispo_date rather than cycle_dispo_date because we only want cycles with final dispositions, and cycle_dispo_date would also catch un-disposed cycles)
...
# save(dispos, file = "Dispos Feb 17.RData")
# save(hist_collapse, file = "History collapse Feb 17.RData")
...
#   ungroup() %>%
#   distinct(subject_id, cycle_dispo_date, .keep_all = T) %>%
#   select(subject_id, cycle_dispo_date, dispo_date, cyc_date, stp_event_date))
# 
# suppressWarnings(dsam %>%
#   group_by(subject_id, cyc_date) %>%
#   mutate(dispo_date = max(dispo_date, na.rm = T)) %>%
...
#   filter(n>1))
#   distinct(subject_id, cyc_date, .keep_all = T)) %>%
#   select(subject_id, dispo_date, cyc_date)
# 
# is.na(as_date(NA))
```
...
df %>%
  filter(court_disp == 1) %>%
  group_by(race) %>%
  summarise(across(contains("arrest_summ"), ~mean(.x>0)))

df %>%
  filter(prior_arrest_summ_f_drug_flag_count >0) %>%
  group_by(race) %>%
  summarise(across(matches("prior_conviction_summ_._drug"), ~mean(.x)))

df %>%
  filter(prior_arrest_summ_f_drug_flag_count >0) %>%
  group_by(race) %>%
  summarise(mean(prior_conviction_summ_f_drug_flag_count/prior_arrest_summ_f_drug_flag_count),
            mean(prior_conviction_summ_m_drug_flag_count/prior_arrest_summ_f_drug_flag_count))

df %>%
  filter(prior_arrest_summ_m_drug_flag_count >0) %>%
  group_by(race) %>%
  summarise(mean(prior_conviction_summ_m_drug_flag_count/prior_arrest_summ_m_drug_flag_count),
            mean(prior_conviction_summ_f_drug_flag_count/prior_arrest_summ_m_drug_flag_count))
...
#temporarily remove hierarchy bc needs to be fixed in collapse
#swap out arrest_sup_vio_flag for on_prob flag
# try more detailed priors
conv <- glm(convicted_flag ~  prior_conviction_summ_f_violent_flag_count + prior_conviction_summ_m_violent_flag_count + prior_conviction_summ_f_property_flag_count + prior_conviction_summ_m_property_flag_count + prior_conviction_summ_f_drug_flag_count + prior_conviction_summ_m_drug_flag_count + prior_conviction_sex_flag_count + on_prob + court_summ_f_violent_flag + court_summ_f_property_flag + court_summ_f_drug_flag + court_sex_flag + court_dv_flag + filed_mcharge_count + filed_fcharge_count  + age + gender +race, data = subset(df, filed_flag == 1), family = 'poisson')
...
df %>%
  select(prior_conviction_summ_f_property_flag_count)
```
...
prison_sent <- glm(sent_prison_flag ~  prior_conviction_summ_f_violent_flag_count + prior_conviction_summ_m_violent_flag_count + prior_conviction_summ_f_property_flag_count + prior_conviction_summ_m_property_flag_count + prior_conviction_summ_f_drug_flag_count + prior_conviction_summ_m_drug_flag_count + prior_conviction_sex_flag_count + on_prob + + conviction_summ_f_violent_flag + conviction_summ_f_property_flag + conviction_summ_f_drug_flag + conviction_sex_flag + conviction_sup_vio_flag + convicted_fcharge_count + convicted_mcharge_count  + age + gender + race, data = subset(df, convicted_flag == 1 & conviction_felony_flag == 1), family = 'poisson')

#not counting misdos (priors or current)
prison_sent <- glm(sent_prison_flag ~  prior_conviction_summ_f_violent_flag_count + prior_conviction_summ_f_property_flag_count +  prior_conviction_summ_f_drug_flag_count +  prior_conviction_sex_flag_count  + on_prob + + conviction_summ_f_violent_flag + conviction_summ_f_property_flag + conviction_summ_f_drug_flag + conviction_sex_flag  + convicted_fcharge_count + scale(age, scale = F)*scale(age_first_dispo, scale = F) + gender + race, data = subset(df, convicted_flag == 1 & conviction_felony_flag == 1 & race %in% c("Black", "White", "Hispanic", "Asian/PI")), family = 'poisson')


prison_sent <- glm(sent_prison_flag ~  prior_conviction_summ_f_violent_flag_count + prior_conviction_summ_f_property_flag_count +  prior_conviction_summ_f_drug_flag_count +  prior_conviction_sex_flag_count  + on_prob + + conviction_summ_f_violent_flag + conviction_summ_f_property_flag + conviction_summ_f_drug_flag + conviction_sex_flag  + convicted_fcharge_count + age + days_since_last_conv:prior_dummy + gender + race, data =  df %>%
  filter(convicted_flag ==1, conviction_felony_flag == 1, race %in% c("Black", "White", "Hispanic", "Asian/PI")) %>%
  mutate(prior_dummy = if_else(prior_convicted_flag_count >0, 1, 0)),
      family = 'poisson')

prison_sent <- glm(sent_prison_flag ~  prior_conviction_summ_f_violent_flag_count + prior_conviction_summ_f_property_flag_count +  prior_conviction_summ_f_drug_flag_count +  prior_conviction_sex_flag_count  + prior_conviction_sup_vio_flag_count + on_prob + + conviction_summ_f_violent_flag + conviction_summ_f_property_flag + conviction_summ_f_drug_flag + conviction_sex_flag  + conviction_sup_vio_flag + convicted_fcharge_count + age*age_first_dispo + days_since_last_conv:prior_dummy + gender*race, data =  df %>%
  filter(convicted_flag ==1, conviction_felony_flag == 1, race %in% c("Black", "White", "Hispanic", "Asian/PI")) %>%
  mutate(prior_dummy = if_else(prior_convicted_flag_count >0, 1, 0)),
      family = 'poisson')
...
summary(df$days_since_last_conv)

df %>%
  select(days_since_last_conv, prior_convicted_flag_count) %>%
  filter(days_since_last_conv > 20000, prior_convicted_flag_count >0)
