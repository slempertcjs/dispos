...
dsam <- vroom("G:/CrimJustice/Dispositions by Race and Ethnicity/DOJ Data 2019/Raw DOJ Data 2019-Empty Variables Removed.csv", n_max = 1000, col_types = c(.default = "c"))
...
Collapse each cycle (second version, using dispo date, Feb 2021)
```{r}
...
conv_county <- glm(convicted_flag ~  prior_conviction_felony_flag_count + prior_conviction_misd_flag_count + arrest_f_offense_type + cyc_age + gender +race + stp_ori_cnty_name, data = df, family = 'poisson')

# summary(conv_county)
conv <- tidy(coeftest(conv_county, vcov. = sandwich)) %>%
...
#without county
prison_sent <- glm(sent_prison_flag ~  prior_conviction_felony_flag_count + prior_conviction_misd_flag_count + conviction_summ_f_violent_flag + conviction_summ_f_property_flag + conviction_summ_f_drug_flag + conviction_sex_flag + conviction_sup_vio_flag + convicted_fcharge_count + convicted_mcharge_count + max_conv_hier + cyc_age + gender +race, data = subset(df, convicted_flag == 1), family = 'poisson')
...
pris_sent <- tidy(coeftest(prison_county, vcov. = sandwich)) %>%
  mutate(relative_risk = round(exp(estimate), 4))
