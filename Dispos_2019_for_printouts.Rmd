...
df %>%
  filter(filed_flag == 1,
         ...
         gender %in% c("M", "F")) %>%
  group_by(race) %>%
  summarise(conviction_of_filings = mean(convicted_flag)) %>%
  ggplot(aes(fill=race, y= conviction_of_filings, x=race)) + 
    geom_bar(position="dodge", stat="identity") +
    ylim(0, 100) +
    geom_text(aes(label=scales::percent(conviction_of_filings, accuracy = .1L)), position=position_dodge(width=0.9), vjust=-0.25)+
...
df %>%
  filter(convicted_flag == 1,
         ...
         gender %in% c("M", "F")) %>%
  group_by(race) %>%
  summarise(felony_conv_of_conv = mean(conviction_felony_flag)) %>%
  ggplot(aes(fill=race, y= felony_conv_of_conv, x=race)) + 
    geom_bar(position="dodge", stat="identity") +
    geom_text(aes(label=scales::percent(felony_conv_of_conv, accuracy = .1L)), position=position_dodge(width=0.9), vjust=-0.25)+
    theme(axis.text.x=element_blank(), legend.position = "none") +
    labs(x = element_blank(), y = element_blank(), fill = element_blank()) + 
    scale_y_continuous(labels = scales::percent, limits=c(0,1))
...
df %>%
  filter(conviction_felony_flag ==1,
         ...
         gender %in% c("M", "F")) %>%
  group_by(race) %>%
  summarise(prison_of_fel_convicted = mean(sent_prison_flag)) %>%
  ggplot(aes(fill=race, y= prison_of_fel_convicted, x=race)) + 
    geom_bar(position="dodge", stat="identity") +
    geom_text(aes(label=scales::percent(prison_of_fel_convicted, accuracy = .1L)), position=position_dodge(width=0.9), vjust=-0.25)+
    theme(axis.text.x=element_blank(), legend.position = "none") +
    labs(x = element_blank(), y = element_blank(), fill = element_blank()) + 
    scale_y_continuous(labels = scales::percent, limits=c(0,1))
